...
public class RejectIssuanceCommandHandler : ICommandHandler<RejectIssuanceCommand, IssuanceDto>
{
    private readonly IIssuanceRepository _issuanceRepository;
    private readonly ILogger<RejectIssuanceCommandHandler> _logger;

    public RejectIssuanceCommandHandler(
        IIssuanceRepository issuanceRepository,
        ILogger<RejectIssuanceCommandHandler> logger)
    {
        _issuanceRepository = issuanceRepository;
        _logger = logger;
    }

    public async Task<IssuanceDto> HandleAsync(RejectIssuanceCommand command, CancellationToken cancellationToken = default)
    {
        _logger.LogInformation("Rejecting issuance request {IssuanceId} with reason: {Reason}",
        ...
        // Reject the issuance
        issuance.Reject(command.RejectedBy, command.Reason, command.Comments);

        // Update in repository
        await _issuanceRepository.UpdateAsync(issuance, cancellationToken);

        _logger.LogInformation("Rejected issuance request {IssuanceId} with status {Status}",
